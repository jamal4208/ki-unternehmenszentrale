...
- Kunden eine bestehende interne Fachroute, ein Werkzeug-/Veröffentlichungsangebot oder Billing erreichen lassen (Phase B Schritt 1 erlaubt ausschließlich das Anlegen/Verfolgen eines einfachen Arbeitsauftrags im Kundenportal `/portal`; es gibt weiterhin keine automatische Ausführung, keinen Agentenstart, keine Toolauswahl, keine Veröffentlichung, kein Billing)
...
- Den OWNER zu einem regulären Pflichtprüfer normaler Kundenprojekte machen (Jamal prüft/genehmigt Marketing-, Design-, Strategie- oder sonstige Kundenaufträge inhaltlich nicht; er greift ausschließlich bei Sicherheit, Missbrauch, rechtlicher Sensibilität, außergewöhnlichen Kosten, technischer Blockade oder expliziter Eskalation über die beiden Ausnahmeaktionen Eskalieren/Stoppen ein)
- Einen `READY_FOR_PROCESSING`-Arbeitsauftrag automatisch an einen Agentenlauf übergeben (Phase B Schritt 1 endet bewusst bei der automatischen Vollständigkeitsentscheidung bzw. den Owner-Ausnahmezuständen `ESCALATED`/`CANCELLED`; die kontrollierte, automatische Übergabe an einen Agentenlauf und die spätere echte fachliche Kundenfreigabe eines Ergebnisses sind eigene, spätere Phasen nach separater Produktentscheidung)
