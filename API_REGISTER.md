...
`server.js` registriert über `server-http-router.js` **63 GET-Routen** und **38 additive POST-Routen** (Execution Bridge seit Phase C gesichert + V7.1 Phase A Dokumente/Backup gesichert `59f985f` + V7.1 Phase B HeyGen-Connector-Pilot gesichert `ff43089` + V7.1 Phase B.1 Mandantenbasis/Ergebnisrückführung gesichert `37e8a28` + V7.1 Phase C Canva-Connector-Pilot, lokal umgesetzt, noch ungesichert). Andere HTTP-Methoden bleiben 405. GET-Routen bleiben read-only. Die POST-Routen schreiben ausschließlich lokal (App-Support-Metadaten bzw. – nach Jamal-Freigabe – ein Fixture-Testrepository); niemals in Health und niemals mit Commit/Push/Deployment.

**V7.1 Phase C (lokal umgesetzt, ungesichert – Commit/Push stehen aus)** ergänzte additiv **3 neue GET-Routen** und **16 neue POST-Routen** für den kontrollierten Canva-Connector-Piloten (`CONTROLLED_CONNECTOR_HANDOFF`, zweiter Medien-Connector nach HeyGen):
...
| 61 | `/api/v71/canva/status` | Canva-Capability-Profil und Pilotstatus | keine HTTP-Eingaben → Capability-Profil, additiver Pilotstatus (`PARTIALLY_CONNECTED`/`CONTROLLED_HANDOFF`), Pilotgrenzen | Read-only; kein API-Key in der Antwort; `noApiKeyStored: true`, `noAutomaticExecution: true` | NEIN | NEIN | V7.1 Phase C (lokal, ungesichert); Quelle `canva-connector.js` |
| 62 | `/api/v71/canva/job-packages` (+ `/api/v71/canva/job-packages/{id}`) | lokal gespeicherte Canva-Designauftragspakete | Query `projectId`/`customerId` optional; Präfixroute für Einzelabruf → Paket-Metadaten inkl. Handoff-Bereitschaft | Read-only; keine Bilder/Videos, keine Secrets in der Antwort | NEIN | NEIN | V7.1 Phase C (lokal, ungesichert); Quelle `canva-store.js` |
| 63 | `/api/v71/canva/backup/export` | additive Canva-Metadatensicherung | keine HTTP-Eingaben → Auftragspaket-/Ergebnis-/Editing-Transaktions-Metadaten-Schnappschuss | Read-only; keine Originaldateien/API-Keys/Tokens/Credentials/privaten Brand-Kit-Assets in der Antwort | NEIN | NEIN | V7.1 Phase C (lokal, ungesichert); Quelle `canva-backup.js` |

### Additive POST-Routen (Execution Bridge seit Phase C gesichert + V7.1 Phase A/B/B.1 gesichert + V7.1 Phase C Canva-Connector-Pilot, lokal ungesichert)
